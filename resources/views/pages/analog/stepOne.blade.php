...
@section('content')
    <div id="app">
        ...
        <div class='step-one'>
            ...
            <button class="my-button primary" @click="stepOneHandle()">
                Submit
            </button>
        </div>
    </div>
    ...
    <script type="module">
        ...
        window.registration = "{{ route('analog.registration', ['hallId' => request('hallId'), 'hash' => request('hash')]) }}";
            ...
            setup() {
                ...
                const stepOneHandle = () => {
                    ...
                    .then((res) => {
                        ...
                        {{--if(res.data.typeUpdate) {--}}
                        {{--    window.location.href = "{{ route('success.index', ['payid' => '__PAY_ID__']) }}".replace('__PAY_ID__', res.data.payId);--}}
                        {{--}else{--}}
                    ...
                    .catch((err) => {
                        inProcess.value = false;
                        document.documentElement.style.overflow = 'auto';
                        errors.value = err.response.data.message ?? err.response.data
                ...
                return {
                    ...
                    handleFileChange, stepOneHandle
                }
            }
        }).mount('#app')
    </script>
